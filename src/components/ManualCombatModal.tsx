import { BookOpen, Target, Swords, Trophy, X } from "lucide-react";
...
export default function ManualCombatModal({
  ...
  setMWon: React.Dispatch<React.SetStateAction<boolean | null>>;
  ...
  function reorderManual(
    ...
    if (from === to || !arr[from] || to < 0 || to >= arr.length) {
      return;
    }
    ...
    >
      ...
      >
        ...
        <p className="text-xs text-white/50 mb-5">
          Renseigne les équipes et le résultat d'un combat déjà joué pour
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-rose-500/20 bg-rose-500/[0.03] p-4">
            ...
            <HeroSlots
              picks={mEnemies}
              onRemove={(id) => setMEnemies(mEnemies.filter((p) => p !== id))}
              onReorder={(from, to) =>
                reorderManual(mEnemies, setMEnemies, from, to)
              }
            ...
            <div className="mt-4">
              <HeroGridPicker
                picks={mEnemies}
                onToggle={(id) => toggleManual(mEnemies, setMEnemies, id)}
                excludeIds={new Set()}
                usage={usage}
          ...
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/[0.03] p-4">
            ...
            <HeroSlots
              picks={mMine}
              onRemove={(id) => setMMine(mMine.filter((p) => p !== id))}
              onReorder={(from, to) => reorderManual(mMine, setMMine, from, to)}
              label="Mon équipe"
            ...
            <div className="mt-4">
              <HeroGridPicker
                picks={mMine}
                onToggle={(id) => toggleManual(mMine, setMMine, id)}
                excludeIds={new Set()}
                usage={usage}
                enabledIds={enabledHeroIds}
              />
