...
type UseHeroManagementParams = {
  ...
  setEnabledHeroIds: React.Dispatch<React.SetStateAction<Set<string>>>;
...
export function useHeroManagement({
  ...
  useEffect(() => {
    setPicks((prev) => prev.filter((id) => enabledHeroIds.has(id)));

    setEditedTeam((prev) => prev.filter((id) => enabledHeroIds.has(id)));

    setMEnemies((prev) => prev.filter((id) => enabledHeroIds.has(id)));

    setMMine((prev) => prev.filter((id) => enabledHeroIds.has(id)));
  }, [enabledHeroIds, setPicks, setEditedTeam, setMEnemies, setMMine]);
